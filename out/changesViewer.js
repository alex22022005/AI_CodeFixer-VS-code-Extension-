...
exports.ChangesViewer = void 0;
const vscode = require("vscode");
class ChangesViewer {
    static async showChangesQuickPick(changedFiles) {
        ...
        quickPick.onDidChangeSelection(async (selection) => {
            if (selection.length > 0) {
                ...
                if (selectedItem.isCloseItem) {
                    ...
                    if (fileName) {
                        vscode.window.showInformationMessage(`📍 Opened: ${fileName} - Select another file or close the viewer`, { modal: false });
                    }
                }
            }
        });
    ...
    static async openFileAndShowChanges(fileUri) {
        try {
            // Open the document
    ...
    static async showChangesWithOptions(changedFiles) {
        if (!changedFiles || changedFiles.length === 0) {
            vscode.window.showInformationMessage('No changes to view');
            return;
        }
    ...
    static async showChangesWebView(changedFiles) {
        if (!changedFiles || changedFiles.length === 0) {
            vscode.window.showInformationMessage('No changes to view');
            return;
        }
        // Create and show webview panel
        const panel = vscode.window.createWebviewPanel('antonysChangesViewer', '📝 Changes Made by Antony\'s AI', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    static getWebviewContent(changedFiles) {
...
exports.ChangesViewer = ChangesViewer;
