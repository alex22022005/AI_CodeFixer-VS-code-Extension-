...
export interface ProjectAnalysisResult {
    ...
    skippedFiles: string[];
    summary: string;
    changedFilesDetails?: ChangedFileDetail[];
}
...
export class ProjectAnalyzer {
    ...
    private changeTracker?: any; // Will be injected
    ...
    constructor(geminiService: GeminiService, changeTracker?: any) {
        this.geminiService = geminiService;
        this.changeTracker = changeTracker;
    }

    async analyzeProject(workspaceFolder: vscode.WorkspaceFolder): Promise<ProjectAnalysisResult> {
    ...
    async analyzeFolder(folderUri: vscode.Uri): Promise<ProjectAnalysisResult> {
    ...
    async fixFolderErrors(folderUri: vscode.Uri): Promise<ProjectAnalysisResult> {
        const files = await this.getCodeFiles(folderUri);
        const results: FileAnalysisResult[] = [];
        ...
        const fixedFiles: string[] = [];
        const skippedFiles: string[] = [];
        const originalContents: Array<{ uri: vscode.Uri, content: string }> = [];
        const changedFilesDetails: ChangedFileDetail[] = [];
        
        // Create a map to track file URI to result mapping
        const fileResultMap = new Map<string, { result: FileAnalysisResult, uri: vscode.Uri }>();
    ...
    async fixProjectErrors(workspaceFolder: vscode.WorkspaceFolder): Promise<ProjectAnalysisResult> {
        const files = await this.getCodeFiles(workspaceFolder.uri);
        const results: FileAnalysisResult[] = [];
    ...
    private async analyzeFile(fileUri: vscode.Uri): Promise<FileAnalysisResult> {
    ...
    private async applyFixToFile(fileUri: vscode.Uri, fixedCode: string): Promise<void> {
        const document = await vscode.workspace.openTextDocument(fileUri);
    ...
    private async getCodeFiles(folderUri: vscode.Uri): Promise<vscode.Uri[]> {
    ...
    private generateSummary(results: FileAnalysisResult[]): string {
